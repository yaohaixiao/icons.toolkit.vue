  ...
  <base-container>
    <base-header
      ...
      <base-breadcrumb>
        ...
        <base-breadcrumb-item current>Meteocons 图标集</base-breadcrumb-item>
      </base-breadcrumb>
    ...
    <base-main padding="outer">
      <article class="article">
        <h1 class="article__h1">{{ METEOCONS.title }}</h1>
        <copyright-meta
          :designer="METEOCONS.designer"
          :homepage="METEOCONS.homepage"
          :license="METEOCONS.license" />
        <h2 class="article__h2">
          {{ `${METEOCONS.title} （${count}）` }}
        </h2>
        <base-grid
          v-if="count > 0"
          :columns="6"
...
import METEOCONS from '../../assets/meteocons'

export default {
  name: 'PageMeteocons',
  componentName: 'PageMeteocons',
  components: {
    BaseContainer,
    BaseHeader,
    BaseMain,
